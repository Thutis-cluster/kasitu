...
let clients = load(STORAGE.clients, seedClients);
let leads = load(STORAGE.leads, seedLeads);
...
function openModal(type, existing=null){
  ...
  modalTitle.textContent=existing ? `Edit ${type}` : `Add ${type}`;
  if(type==='Client') form.innerHTML=clientForm(existing);
  else form.innerHTML=leadForm(existing);
  form.querySelector('input')?.focus();
  form.onsubmit=e=>{e.preventDefault(); if(type==='Client') saveClient(existing); else saveLead(existing);};
}
...
function saveClient(existing){
  const v=values();
  if(existing){ Object.assign(existing,v); } else { clients.push({id:nextClientId(),...v}); }
  save(STORAGE.clients,clients); closeModal(); renderClients();
}
function saveLead(existing){
  const v=values();
  if(existing){ Object.assign(existing,v); } else { leads.push({id:crypto.randomUUID(),date:new Date().toISOString().slice(0,10),...v}); }
  save(STORAGE.leads,leads); closeModal(); renderLeads();
}
function nextClientId(){const year=new Date().getFullYear(); const nums=clients.map(c=>Number(String(c.id).split('-').pop())).filter(Number.isFinite); const next=(Math.max(0,...nums)+1).toString().padStart(3,'0'); return `KAS-${year}-${next}`;}
...
function bindNavigationButtons(){
  document.querySelectorAll('[data-go]').forEach(b=>b.addEventListener('click',()=>{const target=b.dataset.go;document.querySelector(`[data-view="${target}"]`)?.click();}));
...
document.addEventListener('click',e=>{
  const clientId=e.target.closest('[data-edit-client]')?.dataset.editClient;
  if(clientId){openModal('Client',clients.find(c=>c.id===clientId));return;}
  const leadId=e.target.closest('[data-edit-lead]')?.dataset.editLead;
  if(leadId){openModal('Lead',leads.find(l=>l.id===leadId));return;}
